...
export const cocktail = (name,input)=>{
    
         if (name ==="name" &&( input.length < 1 || input.length > 50)) {
          cocktailObj.name.message =`The ${name} should be between 1 and 50 charecters`
          cocktailObj.name.err=true  
        } else if(name ==="name" && (input.length > 1 || input.length <50)) {
          cocktailObj.name.message =``
          cocktailObj.name.err=false
        }  else if (name ==="recipe" && (input.length < 5 || input.length > 50)) {
          cocktailObj.recipe.message =`The ${name} should be between 5 and 50 charecters`
          cocktailObj.recipe.err=true  
        } else if (name ==="recipe" && (input.length > 5 || input.length < 50)){
          cocktailObj.recipe.message =``
          cocktailObj.recipe.err=false
        }
